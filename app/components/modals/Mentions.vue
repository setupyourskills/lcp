...
div.mentions-modal
  ComponentModal(
    ...
    div.mentions-modal__container(
      v-clean-html="JSON.parse(component_info.component_name)[lang]"
      @click="clickHandler($event, true, 'mentions')"
    )
    div.mentions-modal__button
      ComponentButton(
        :title="JSON.parse(component_button.component_name)[lang]"
        @click="setModalState('mentions', false)"
      )
</template>
...
const { data: modalBlocks } = await useFetch<ISectionFullRow[]>("/api/view/mentions_modal_view");
