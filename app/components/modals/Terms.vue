...
div.terms-modal
  ComponentModal(
    ...
    div.terms-modal__container(
      v-clean-html="JSON.parse(component_info.component_name)[lang]"
      @click="clickHandler($event, true, 'terms')"
    )
    div.terms-modal__button
      ComponentButton(
        :title="JSON.parse(component_button.component_name)[lang]"
        @click="setModalState('terms', false)"
      )
</template>
...
const { data: modalBlocks } = await useFetch<ISectionFullRow[]>("/api/view/terms_modal_view");
